/*
Function name: oni_core_fnc_Spawner_SpawnerInit
Author: Mr H.
...
{
	[_x , "CHECKBOX",gettext (configfile >> "CfgMods" >> _x >> "name"), "ONI Spawner Allowed DLCs", true,1, { call oni_core_fnc_Spawner_AllowedDLCs;}] call cba_settings_fnc_init;
	Diag_log format ["ONI Spawner AllDLCs -SpawnerInit.sqf: %1", str _x];
} forEach _array;
...

{
	if (_x == "Default") then 
	{
		 _deBuGMess = format ["ONI Spawner - error faction %1 is named 'default' faction will not be included to avoid conflicts with cba", (str _x)];
		 Diag_log _deBuGMess;
	 } 
	 else 
	 {
		[_x , "CHECKBOX",gettext (configfile >> "CfgFactionClasses" >> _x >> "displayname"), "ONI Spawner Allowed Factions", true,1, { call oni_core_fnc_Spawner_AllowedFactions;}] call cba_settings_fnc_init;
		Diag_log format ["ONI Spawner AllFactions -SpawnerInit.sqf: %1", str _x];
	};
