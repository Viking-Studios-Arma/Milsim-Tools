[] spawn {
	...
	if (isServer) then {
		_loadoutUnits = allUnits select {_x getVariable ["oni_core_isLoadout", false]};
		oni_core_loadouts = [];
		{
			_role = _x getVariable "oni_core_loadout_role";
			oni_core_loadouts pushBack [_role, getUnitLoadout _x];
			deleteVehicle _x;
...
[{isClass (configFile >> "CfgPatches" >> "zen_custom_modules")},{
	["ONI Modules", "Add Barracks Functions",
	{
...
addMissionEventHandler ["Map", {
	...
	if (_mapIsOpened) then {
		...
		oni_core_restoredVolume = missionNamespace getVariable ["acex_volume_initialGameVolume", soundVolume];
	...
	if ({_x == _gesture} count _acceptedGestures > 0 && _player distance _target < 10) then {
		...
		[format["%1 told %2 to follow using a %3 gesture", _player, _target, _gesture], "core\XEH_postInit.sqf"] call oni_core_fnc_log;
			...
			private _following = [_target, _player] spawn {
				...
				while {(_target getVariable ["oni_core_following", false]) isEqualTo _player} do {
					if (_index > 30) exitWith {
						_target setVariable ["oni_core_following", nil, true];
					};
